...
function App() {
	...
	const createCelles = useCallback (function () {
		const squares = [];
		...
		setTimeout(function () {
			...
			const newCells = cells.map((cell) => {
				...
				cordinates.forEach((cordinate) => {
					if (alives.some((alive) =>
								alive.row == cordinate.row &&
								alive.column == cordinate.column 
						)) {
							aliveCounter++
					}
				});
				if(aliveCounter === 3) return {...cell,aliveCounter, isAlive:true,}
				if(aliveCounter ===2 || aliveCounter === 3) return {...cell,aliveCounter}
				if(aliveCounter < 2){  return {...cell,aliveCounter, isAlive:false}}
